...
function isotopologues_elements(precise::Val, element_dictionary::Dict, msfix, abundance, abtype, threshold; normalize = true)
    ...
    max_proportion, max_dictionary = maximal_abundance_elements(precise, element_dictionary)
...
function rec_addminusisotopes!(
    ...
    if iter && (prev_abundance >= threshold || all(current))
        next_abundance = rec_addminusisotopes!(element_vec, mass_vec, abundance_vec, element_dictionary, element_isotope_pair, isotope_position + 1, prev_mass, prev_abundance, threshold, current, precise)
    ...
    if first(current)
        ...
        while cni > 0
            ...
            elseif iter_e && new_abundance >= abundance
                next = backward_max_abundance >= threshold ? (false, true) : (false, false)
            ...
            elseif abundance >= threshold && iter
                ...
                push!(mass_vec, mass)
                backward_next_abundance = rec_addminusisotopes!(element_vec, mass_vec, abundance_vec, element_dictionary, element_isotope_pair, isotope_position + 1, mass, abundance, threshold, next, precise)
    ...
    if last(current)
        ...
        while cne > 0
            ...
            elseif iter_e && new_abundance >= abundance
                ...
                next = forward_max_abundance >= threshold ? (false, true) : (false, false)
            elseif new_abundance >= abundance
                next = (false, false)
            elseif iter_e
                # newi = last(element_isotope_pair[isotope_position + 1])
            ...
            elseif abundance >= threshold && iter
                ...
                push!(abundance_vec, abundance)
                push!(mass_vec, mass)
                forward_next_abundance = rec_addminusisotopes!(element_vec, mass_vec, abundance_vec, element_dictionary, element_isotope_pair, isotope_position + 1, mass, abundance, threshold, next, precise)
...
function rec_addminusisotopes_iter!(
    ...
    iter_e = iter && first(element_isotope_pair[isotope_position + 1]) == e
    if iter && (prev_abundance >= threshold || all(current))
        next_abundance = rec_addminusisotopes_iter!(element_vec, mass_vec, abundance_vec, preab_vec, element_dictionary, element_isotope_pair, isotope_position + 1, prev_mass, prev_abundance, prev_preab, threshold, current, precise)
        max_abundance = max(max_abundance, next_abundance)
    end
    ne = get(element_dictionary, e, 0)
    ni = get(element_dictionary, i, 0)
    if first(current)
        ...
        while cni > 0
            ...
            elseif abundance >= threshold && iter
                ...
                push!(abundance_vec, abundance)
                push!(mass_vec, mass)
                backward_next_abundance = rec_addminusisotopes_iter!(element_vec, mass_vec, abundance_vec, preab_vec, element_dictionary, element_isotope_pair, isotope_position + 1, mass, abundance, preab, threshold, next, precise)
                backward_max_abundance = max(abundance, backward_next_abundance)
            elseif abundance >= threshold
                element_dictionary[i] = cni
                push!(element_vec, get_isotope_vec(element_dictionary))
    ...
    if last(current)
        ...
        while cne > 0
            ...
            elseif abundance >= threshold && iter
                ...
                push!(abundance_vec, abundance)
                push!(mass_vec, mass)
                forward_next_abundance = rec_addminusisotopes_iter!(element_vec, mass_vec, abundance_vec, preab_vec, element_dictionary, element_isotope_pair, isotope_position + 1, mass, abundance, preab, threshold, next, precise)
                forward_max_abundance = max(abundance, forward_next_abundance)
            elseif abundance >= threshold
                element_dictionary[i] = cni
                push!(element_vec, get_isotope_vec(element_dictionary))
