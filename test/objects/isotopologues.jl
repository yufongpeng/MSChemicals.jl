...
it1 = Isotopologues(icglc[1]; abundance = 1e5, threshold = crit(1e1, 1e-2), sort = true)
it2 = Isotopologues(ioncore(icglc[1]); abundance = 1e5, abtype = :total, threshold = crit(1e1, 1e-2), sort = true)
it3 = Isotopologues(ipsi2[1]; abtype = :total, threshold = crit(1e-3, 1e-3), sort = true)
it4 = Isotopologues("C6H12O6"; abundance = 1e5, abtype = :total, threshold = crit(1e1, 1e-2), sort = true)
...
itit1 = TandemIsotopologues(cp1; abundance = 1e5, threshold = crit(1e1, 1e-2), transmission = 0.3, sort = true)
itit2 = TandemIsotopologues(chemicalformula(icps[1]) => chemicalformula(last(chemicaltransition(cp1))); abundance = 1e5, threshold = crit(1e1, 1e-2), transmission = 0.3, chemicalparser = ChemicalTransitionParser(), sort = true)
itit3 = TandemIsotopologues(ChemicalSeries(AdductIon(cps, "[M-H]-"), lossserine); abundance = 1e5, threshold = crit(1e1, 1e-2), transmission = 0.5, sort = true)
itit4 = TandemIsotopologues(string("[", chemicalformula(cps), "-H]-") => "-C3H5NO2"; abundance = 1e5, threshold = crit(1e1, 1e-2), transmission = 0.5, sort = true)
itit5 = TandemIsotopologues(ChemicalSeries(ips[1], AdductIon(ioncore(ips[1]).fa1, "[M-H]-")); abtype = :total, threshold = crit(1e-8, 1e-8), transmission = 0.3, sort = true)
itit6 = TandemIsotopologues(ChemicalSeries(ipsi1[3], LossSerine(), SN1Acyl()); abtype = :total, abundance = 1e5, threshold = crit(1e1, 1e-2), transmission = 0.5, precise = true, sort = true)

itit7 = Isotopologues("[S8]2-" => "[S7]-"; abtype = :total, sort = true)
itit8 = Isotopologues("[C2H5[13C]OO]-" => "[C2H5]-"; abtype = :total, sort = true)
itit9 = Isotopologues("[C2H5[13C]OO]-" => "-[13C]O2"; abtype = :total, sort = true)
itit10 = Isotopologues("[C2H5[12C]OO]-" => "-[12C]O2"; abtype = :total, sort = true)

itit11 = Isotopologues("[C2H5[12C]OO]-" => "[-H2O-CO]" => "+H2O"; abtype = :input, sort = true)
itit12 = TandemIsotopologues("[C2H5[12C]OO]-" => "[-H2O-CO]" => "+H2O"; abtype = :input, sort = true)
itit13 = TandemIsotopologues("[C2H5[12C]OO]-" => "-[CO]+" => "+CO2"; abtype = :input, sort = true)
itit14 = TandemIsotopologues("[C2H5[12C]OO]-" => "-CO" => "+[CO2]-"; abtype = :input, sort = true)
gitit12 = group_isotopologues(itit12)
gitit13 = group_isotopologues(itit13)
gitit14 = group_isotopologues(itit14)

itit15 = TandemIsotopologues(repeat([icps[3]], 1000); product = [lossserine], abundance = [1e5], threshold = crit(1e1, 1e-2), proportion = [0.5], transmission = 0.3, threading = true, sort = true)
itit16 = TandemIsotopologues(Table(; Chemical = repeat([icps[3]], 1000), Product = repeat([[lossserine]], 1000), Abundance1 = repeat([1e5], 1000), Proportion = repeat([[0.5]], 1000), Transmission = repeat([0.3], 1000)); threshold = crit(1e1, 1e-2), threading = true, sort = true)
itit17 = TandemIsotopologues("[C2H5[12C]OO]-" => "-CO" => "-H2O"; abtype = :input, sort = true)
itit18 = TandemIsotopologues("[C2H5[12C]OO]-" => "-CO" => "OH"; abtype = :input, sort = true)
itit19 = TandemIsotopologues("[C200H500[12C]O200]-" => "-C100O100" => "-O50H50"; abtype = :input)
itit20 = TandemIsotopologues("[C200H500[12C]O200]-" => "-C100O100" => "[C100H450[12C]O50]-"; abtype = :input)
itit21 = TandemIsotopologues("[C200H500[12C]O200]-" => "[C100H500[12C]O100]-" => "-O50H50"; abtype = :input)
itit22 = TandemIsotopologues("[C200H500[12C]O200]-" => "[C100H500[12C]O100]-" => "[C100H450[12C]O50]-"; abtype = :input)
